...
# 자주 수정하는 parameter 앞으로
result_numpy_path = '1205_peak_timing'
...
def state_to_nn(state):  # nn에 전달해주는 값은  [2]'stock level',[5]'customer' // 'last time sales', '
    ...
    #    y = to_tensor(norm_state[4]) #buyer
    #    z = to_tensor(x, y)
    #    z = y
    return x
...
# 집행하는 것을 기준으로만 진행하는 함수. 진행하지 않을 때는 해당 것을 사용하지 않는다.
# [0]'stock level', [1]'price', [2]'last time sales' [3]'time index', [4]'index'
...
def env_step(t, state, action):
    ...
    # 구매 발생
        ...
        # 재고가 없을 때
            ...
            next_state[0] = 0
            next_state[1] = 0
            next_state[2] = purchase_group
            sales = purchase_group
            C2 = n * c2
            next_state[3] = t
    ...
    # 재고보충 발생
        ...
        C1 = c1 * state[0]/(24-T) # 영업하지 않는 동안 남은 재고에 관해 holding cost 발생.
        next_state[3] = t
...
# train
#return_trace, loss_trace, train_visit_state = [[] for i in range(3)]
...
# test할 때 같은 순서로 customer가 들어오도록 함
#si_for_test = [si_list(num_episodes_test) for _ in range(test_trial)]
...
def env_step_random(t, state, action):
    ...
    if t == 3:
        if state[1] >= p_t_high():  # 가격 바운더
            purchase_group = 2 * customer[0] if state[0] > 0 else 0
            next_state[0] = state[0] - purchase_group if state[0] >= purchase_group else 0  # 재고 빼
            ...
            purchase_group = 2 * customer[1] if state[0] > 0 else 0
            next_state[0] = state[0] - purchase_group if state[0] >= purchase_group else 0  # 재고 빼
            ...
            purchase_group = 2 * customer[2] if state[0] > 0 else 0
            next_state[0] = state[0] - purchase_group if state[0] >= purchase_group else 0  # 재고 빼
            if next_state[1] > state[1]:  # 다음 시점 가격이 높아지면 안되니까.
                next_state[1] = state[1] if t != 0 else action_list[10]
            else:
    ...
    # 재고보충 발생
        ...
        sales = 0
        C3 = c3 * (q-state[0])  # 주문비용만 발생
        C1 = c1 * state[0]/(24-T) # 영업하지 않는 동안 holding cost 발생.
        next_state[3] = t
        # 이 다음에는 원래대로 돌아간다.
